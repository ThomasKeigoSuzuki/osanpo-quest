  ...
  return (
    <div className="relative h-dvh w-full overflow-hidden" style={{ maxWidth: 448, margin: "0 auto" }}>
      ...
      {[20, 35, 55, 72, 88].map((left, i) => (
        <div
          key={i}
          className="absolute z-[2] h-1.5 w-1.5 rounded-full bg-white"
          style={{
            bottom: "15%",
            left: `${left}%`,
            opacity: 0,
            animation: `windParticle 3s ease-in-out infinite ${i * 1.1}s`,
          }}
        />
      ))}
      ...
      {/* z-10: タイトルブロック */}
      <div
        className="absolute left-0 right-0 z-10 text-center transition-all duration-1000"
        style={{
          bottom: "30%",
      ...
      >
        ...
        <h1
          className="font-wafuu mt-4 text-3xl font-bold text-gold"
          style={{ animation: showTitle ? "starGlow 2s ease-in-out infinite" : undefined }}
        >
          おさんぽクエスト
        </h1>
        <p className="mt-2 text-xs" style={{ color: "rgba(232,184,73,0.8)" }}>
          忘れられた風の神と、今日を歩く
        </p>
      ...
      {/* z-20: CTAボタン群 */}
      <div
        className="absolute bottom-12 left-0 right-0 z-20 px-6 safe-bottom transition-all duration-700"
        style={{
      ...
      >
        <button
          ...
          style={{
            ...
            boxShadow: "0 4px 24px rgba(232,184,73,0.5)",
          }}
        >
          {loading ? "準備中..." : "はじめる"}
        </button>
        ...
        <button
          ...
          className="mt-2 w-full py-2 text-center text-[11px] transition"
        ...
        {showEmail && (
          <form onSubmit={handleMagicLink} className="mt-2 flex gap-2" style={{ animation: "dialogueFadeIn 0.3s ease-out" }}>
            <input
              ...
              placeholder="メールアドレス"
              required
              className="min-w-0 flex-1 rounded-lg px-3 py-2.5 text-xs outline-none"
              style={{ background: "rgba(0,0,0,0.4)", color: "var(--color-text)", border: "1px solid var(--color-border)" }}
            />
